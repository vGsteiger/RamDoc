...
- **macOS `.dmg` disk images** (e.g., `RamDoc_<version>_aarch64.dmg`, `RamDoc_<version>_x86_64.dmg`) for manual installation
- **macOS `.app` bundle tarballs** (e.g., `RamDoc_<version>_aarch64.app.tar.gz`, `RamDoc_<version>_x86_64.app.tar.gz`) for the auto-updater
- **latest.json** - Auto-updater manifest with signatures and download URLs

## First Release

If no previous tags exist, the workflow treats the initial version as `v0.0.0` and applies the usual bump rules. With the default patch bump, the first release will typically be `v0.0.1` unless a `minor` or `major` label is applied to the first merged PR.
