...
<div class="p-8 max-w-2xl">
  <h1 class="text-2xl font-bold text-gray-100 mb-6">Kalender</h1>

  <!-- Week navigation -->
  <div class="flex items-center gap-3 mb-6">
    <button
      onclick={prevWeek}
      class="px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-100 text-sm transition-colors"
    >
      ‹
    </button>
    <span class="text-gray-200 text-sm font-medium flex-1 text-center">{weekLabel}</span>
    <button
      onclick={nextWeek}
      class="px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-100 text-sm transition-colors"
    >
  ...
  {#if weekSessions.length === 0}
    <p class="text-gray-400 text-sm">Keine Sitzungen diese Woche.</p>
  {:else}
    <div class="space-y-6">
          ...
          <div class="space-y-2">
            {#each sessions as item}
              <button
                ...
                class="w-full text-left bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg px-4 py-3 flex items-center gap-3 transition-colors"
              >
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium text-gray-100 truncate">{item.patient_name}</p>
                </div>
                <span class="text-xs px-2 py-0.5 rounded-full bg-blue-900 text-blue-200 shrink-0">
                  {SESSION_TYPE_LABELS[item.session.session_type] ?? item.session.session_type}
                </span>
                {#if item.session.duration_minutes}
                  <span class="text-xs text-gray-400 shrink-0">{item.session.duration_minutes} Min.</span>
                {/if}
              </button>
            {/each}
          </div>
