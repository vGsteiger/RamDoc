...
fn main() {
    ...
    audit::log(&conn, AuditAction::Create, "patient", Some("patient-123"), None).unwrap();
    audit::log(&conn, AuditAction::View, "patient", Some("patient-123"), None).unwrap();
    audit::log(&conn, AuditAction::Update, "patient", Some("patient-123"), Some("fields: first_name")).unwrap();
    ...
    let patient_entries = audit::query_log(&conn, Some("patient"), None, None, None, 100, 0).unwrap();
    ...
    audit::log(&conn, AuditAction::View, "patient", Some("patient-789"), None).unwrap();
    let specific_entries = audit::query_log(&conn, None, Some("patient-123"), None, None, 100, 0).unwrap();
    assert_eq!(specific_entries.len(), 3, "Should have 3 entries for patient-123");
    println!("✓ Test 3 passed: Filtering by entity ID works");

    // Test 4: Pagination
    for i in 0..10 {
        audit::log(&conn, AuditAction::View, "test", Some(&format!("test-{}", i)), None).unwrap();
    }
    ...
    audit::log(&conn, AuditAction::Update, "patient", Some("patient-123"),
        Some("fields: first_name,last_name,date_of_birth")).unwrap();
    let entries_with_details = audit::query_log(&conn, None, Some("patient-123"), None, None, 100, 0).unwrap();
